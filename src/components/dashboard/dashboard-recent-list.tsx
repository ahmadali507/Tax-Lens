...
interface DashboardRecentListProps {
  recentTaxSlips: TaxSlip[];
  categoryBreakdown: Array<{ category: string; amount: number; percentage: number }>;
}

export function DashboardRecentList({ recentTaxSlips, categoryBreakdown }: DashboardRecentListProps) {
    ...
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <Card className="glass glass-border">
        <CardHeader>
          ...
          <CardDescription>Your tax contributions by category</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card className="glass glass-border">
        <CardHeader>
          ...
          <CardDescription>Your latest tax slip uploads</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {recentTaxSlips.map((slip) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{formatDate(slip.date)}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold text-foreground">{formatCurrency(slip.amount)}</p>
                  </div>
                </div>
              ))}
            </div>
